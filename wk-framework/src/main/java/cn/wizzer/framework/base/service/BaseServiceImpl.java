...
//        if(judgeRelervanceBeforeDelete(refValue,id)){
        if(judgeRelervanceBeforeBatchDelete(refValue,new String[]{id})){
          return this.dao().delete(this.getEntityClass(), id);
        }
        return 0;
    }
    ...
    public void delete(String[] ids) {
        //20180316zhf1531
        String refValue = this.getEntityClass().getSimpleName();
        if(judgeRelervanceBeforeBatchDelete(refValue,ids)){
            this.dao().clear(this.getEntityClass(), Cnd.where("id", "in", ids));
            return ;
        }
    }
     ...
     * @param ids        删除对象的ids
     */
    public boolean judgeRelervanceBeforeBatchDelete(String refValue, String[] ids) {
        for (Class<?> klass : Scans.me().scanPackage("cn.wizzer.app")) {
            if (klass.getAnnotation(Table.class) != null) {
                String comment="";
                if(klass.getAnnotation(Comment.class)!=null){
                    comment = klass.getAnnotation(Comment.class).value();
                }
                ...
                while (tempClass != null) {//当父类为null的时候说明到达了最上层的父类(Object类).
                    ...
                    //3.遍历类的所有属性
                        ...
                        if (ref != null &&  refValue.equals(ref.value().getSimpleName())) {
                            //4.说明当前对象存在关联关系
                            String tableName = klass.getSimpleName();
                            tableName =   new StringBuilder(80).append(Character.toLowerCase(tableName.charAt(0))).append(tableName.substring(1,tableName.length())).toString();
                            int count = this.count(tableName, Cnd.where(f.getName(), "in", ids));
                            if (count <= 0) {
                                //当前对象没有被引用
                                //baseService.vDelete(id);
                                return true;
                            }else{
                                throw new ValidatException("数据正在被["+("".equals(comment)?tableName:comment)+"]引用");
                            }
                        }
                    }
            }
        }
        //扫描的所有类没有Ref标签,没有关联关系
        return true;
    }
    ...
    /**
     *   //供消息中心审批处理调用，由具体的业务对象继承实现
     * @param objMap  处理界面上所有可传入表单元素
     * @param type 同Msg_option的type属性
     */
    @Override
    ...
    //20180625zhf1655
    public void deleteOldPicture(String oldPic, String imgPath) {
        //删除原来的图片
            //一张
            String old = oldPic;
            String[] oldRelaPath = old.split("\\/");
            if(oldRelaPath.length > 0){
                new File(imgPath+oldRelaPath[oldRelaPath.length-1]).delete();
            }
    }
