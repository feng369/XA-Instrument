...
public class BasePersonController{
    ...
    @Ok("json")
    @RequiresPermissions("platform.base.person.select")
    public Object data(@Param("flag")boolean flag,  @Param("keyword")String keyword,@Param("selectForm") String selectForm,@Param("length") int length, @Param("start") int start, @Param("draw") int draw, @Param("::order") List<DataTableOrder> order, @Param("::columns") List<DataTableColumn> columns) {
		Cnd cnd = Cnd.NEW();
        ...
        if(selectForm !=null){
            ...
            if(!Strings.isBlank(keyword)){
                ...
                cnd.exps("jobNumber","LIKE","%"+keyword+"%").or("personname","LIKE","%"+keyword+"%");
            }
