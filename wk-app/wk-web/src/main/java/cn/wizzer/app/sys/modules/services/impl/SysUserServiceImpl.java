...
public class SysUserServiceImpl extends BaseServiceImpl<Sys_user> implements SysUserService {
    ...
    public Map getUserInfo(Sys_user curUser, String clientId){
        Cnd cnd = Cnd.NEW();
        Map map = new HashMap();
        ...
        this.update(Chain.make("clientId",clientId),Cnd.where("id","=",baseCnctobj.getUserId()));
        return map;
    }
    ...
    public Map doLoginByMobile(Sys_user user) {
        int count = user.getLoginCount() == null ? 0 : user.getLoginCount();
        this.update(Chain.make("loginIp", user.getLoginIp()).add("loginAt", (int) (System.currentTimeMillis() / 1000))
        ...
        List<base_cnctobj> baseCnctobjs = baseCnctobjService.query(Cnd.where("userId", "=", user.getId()));
        if(baseCnctobjs.size() > 0){
            base_cnctobj baseCnctobj = baseCnctobjs.get(0);
            base_person basePerson = basePersonService.fetch(Cnd.where("id", "=", baseCnctobj.getPersonId()));
            map.put("airportId",basePerson.getAirportid());
        }
        return map;
    }
