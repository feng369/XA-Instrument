...
public class SysRegistauditController{
    ...
    @Ok("json:full")
    @RequiresPermissions("sys.manager.registaudit")
    public NutMap getRegistList(@Param("result")int result, @Param("length") int length, @Param("start") int start, @Param("draw") int draw, @Param("::order") List<DataTableOrder> order, @Param("::columns") List<DataTableColumn> columns) {
    	return sysRegistauditService.getRegistList(result,length, start, draw, order, columns, Cnd.NEW());
    }
