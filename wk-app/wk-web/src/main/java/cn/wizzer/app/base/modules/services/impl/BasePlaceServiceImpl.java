...
//    }
    ...
    public List<Map<String, Object>> bindDDLByMobile(String airportId, Integer pagenumber, Integer pagesize) {
        pagesize = 30;//IOS要求
        List<Map<String, Object>> list = new ArrayList<>();
        if (!Strings.isBlank(airportId)) {
            List<base_place> place = this.query(Cnd.where("airportId", "=", airportId).orderBy("placecode", "ASC"), "airport|area|customer|type|ctrl|person", StringUtil.getPagerObject(pagenumber, pagesize));
            if (place.size() > 0) {
