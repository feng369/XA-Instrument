...
public class SysWxServiceImpl extends BaseServiceImpl<Sys_wx> implements SysWxService {
    ...
    //暴露userid出来
    public String addWXUser(String username,String mobile){
        String userid = null;
        try{
            String corpid = Globals.WxCorpID;
            if(Strings.isBlank(corpid)){
            ...
            if(!Strings.isBlank(username)){
                ...
                if(jsonObject != null){
                    ...
                    //同步到本地sys_wx_user表中
                    Sys_wx_user sysWxUser = new Sys_wx_user();
                    sysWxUser.setName(username);
                    sysWxUser.setUserid(userid);
                    sysWxUser.setMobile(mobile);
                    sysWxUser.setBindPerson(false);
                    sysWxUser.setCorpid(Globals.WxCorpID);
                    sysWxUser.setStatus("1");
                    sysWxUser.setDepartment(Arrays.toString(depts));
